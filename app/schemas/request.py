...
class HistoricalRequestSchema(BaseModel):
    ...
    source: SourceEnum | None = Field(default=None,examples=["tgju"])
    limit: int | None = Field(default=100,ge=1,le=1000)


class ChartRequestSchema(BaseModel):
    source: SourceEnum = Field(default="tgju",examples=["tgju"])
...
class PredictionRequestSchema(BaseModel):
    source: SourceEnum = Field(default=SourceEnum.TGJU,examples=["tgju"])